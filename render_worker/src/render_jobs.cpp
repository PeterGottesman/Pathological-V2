...
void RenderJobs::AddJob(std::string uuid, std::shared_ptr<Job> job) { this->jobMap[uuid] = job; }

void RenderJobs::UpdateJobStatus(std::string uuid, render_server::Status status) {
    this->jobMap[uuid]->setStatus(status);
}

Status RenderJobs::FetchJobStatus(std::string uuid) { return this->jobMap[uuid]->getStatus(); }
