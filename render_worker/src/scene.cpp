...
Scene::Scene(const VulkanContext& ctx, const std::vector<Vertex>& vertices, const std::vector<uint32_t>& indices,
    ...
    // Create index buffer
    m_indexBuffer =
        std::make_unique<Buffer>(ctx.allocator(), sizeof(uint32_t) * indices.size(),
    ...
    m_materialBuffer = std::make_unique<Buffer>(
        ctx.allocator(), sizeof(Material) * materials.size(),
    ...
    m_materialIndexBuffer = std::make_unique<Buffer>(
        ctx.allocator(), sizeof(uint32_t) * materialIndices.size(),
        vk::BufferUsageFlagBits::eStorageBuffer | vk::BufferUsageFlagBits::eShaderDeviceAddress,
        VMA_MEMORY_USAGE_CPU_TO_GPU, VMA_ALLOCATION_CREATE_HOST_ACCESS_SEQUENTIAL_WRITE_BIT);
