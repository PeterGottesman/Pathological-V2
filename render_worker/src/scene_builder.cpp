...
static glm::mat4 getGltfNodeTransform(const tinygltf::Node& gltfNode) {
    ...
    if (gltfNode.matrix.size() == 16) {
        ...
        if (gltfNode.rotation.size() == 4) {
            ...
            rotation = glm::quat(static_cast<float>(gltfNode.rotation[3]), static_cast<float>(gltfNode.rotation[0]),
    ...
    if (gltfNode.mesh >= 0) {
        ...
        if (!gltfMesh.primitives.empty()) {
            ...
            if (primitive.mode != TINYGLTF_MODE_TRIANGLES) {
                std::cerr << "Warning: Skipping non-triangle primitive in mesh " << gltfMesh.name << "\n";
                return;
            }
