...
// from the server.
std::string RenderWorkerClient::RenderJob(const std::shared_ptr<RenderRequest>& render) {
    ...
    request.set_time(static_cast<float>(
        render->getAnimationRuntimeInFrames()));  // ****I'm ASSUMING this is what is from the frontend
